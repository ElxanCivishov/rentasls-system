...
export const CustomTextField: FC<TextFieldProps> = function ({ inputDetails, ...buildIndInputProps }) {
    ...
    const { checkValidate, label, field, required, disabled, resetForm, errorMessage, value, specificText, key, type, maxLength } = inputDetails;
    ...
    return (
        <FloatLabel className={buildIndInputProps.status} label={label} value={value} specificText={specificText} isRequired={required}>
            <>
                {isNumberInput ? (
                    <InputNumber inputMode='decimal' {...inputProps} controls={false} name={key} />
                ) : (
                    <Input {...inputProps} {...buildIndInputProps} name={key} />
                )}
